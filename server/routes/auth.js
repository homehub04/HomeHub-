...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const info = db
      .prepare(`INSERT INTO users (email, phone, password_hash, name, role) VALUES (?,?,?,?,?)`)
      .run(email, phone, password_hash, name, role);

    const user = db.prepare('SELECT * FROM users WHERE id = ?').get(info.lastInsertRowid);
    const token = signToken(user);

    // Account is created and the token is issued regardless of OTP delivery —
    // if email/SMS sending fails (bad SMTP creds, provider down), the user can
    // still verify via "resend" once the underlying issue is fixed, instead of
    // losing the whole signup.
    let otpWarning = null;
    try {
      await sendEmailOtp(email, 'signup');
    } catch (err) {
      console.error('sendEmailOtp failed:', err.message);
      otpWarning = 'Account created, but the verification email could not be sent. Try "Resend code".';
    }
    try {
      await sendPhoneOtp(phone, 'signup');
    } catch (err) {
      console.error('sendPhoneOtp failed:', err.message);
    }

    res.status(201).json({
      message: otpWarning || 'Account created. Check your email and phone for verification codes.',
      warning: otpWarning,
      token,
...
router.post('/resend', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Could not send the code right now. Check server logs.' });
  }
});
